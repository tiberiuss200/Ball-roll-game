...
public class AIChase : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hitInfo, seeDistance, ~EnemyLayer) && (hitInfo.transform.tag == "Player")) { //If the cat can see the player
        ...
        else if (timer >= 0){//Otherwise, if the cat is still chasing the player
            if (!reachedPlayerPos)
            {
            ...
            {
                Debug.Log("Starting roaming script");
    ...
    void checkWalls(ref float[] array) //A function to check for the walls around the cat, and lower the weight for the directions towards the walls and around those directions
    {
        for (int i = 0;i < 16;i++)
        {
            ...
            if (Physics.Raycast(currentWallChecker, out wallCheckInfo, 1f, ~EnemyLayer) && (wallCheckInfo.transform.tag == "Walls")){ //The wall has to be within 2 units to be detected

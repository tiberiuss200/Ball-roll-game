    ...
    internal class BurstAotCompiler : IPostBuildPlayerScriptDLLs
    {
        ...
        private void OnPostBuildPlayerScriptDLLsImpl(BuildReport report)
        {
                ...
                )
            {
                ...
                {
                    commonOptions.Add(GetOption(OptionLinkerOptions, $"vs-version={EditorUserBuildSettings.wsaUWPVisualStudioVersion}"));
                }

                if (!string.IsNullOrEmpty(EditorUserBuildSettings.wsaUWPSDK))
                {
                    commonOptions.Add(GetOption(OptionLinkerOptions, $"target-sdk-version={EditorUserBuildSettings.wsaUWPSDK}"));
                }
            }
...
#if UNITY_2021_2_OR_NEWER
                if (PlayerSettings.GetScriptingBackend(NamedBuildTarget.FromBuildTargetGroup(BuildPipeline.GetBuildTargetGroup(buildTarget))) == ScriptingImplementation.IL2CPP)
#else
        ...
        private static List<BurstOutputCombination> CollectCombinations(TargetPlatform targetPlatform, TargetCpus targetCpus, BuildReport report)
        {
            ...
            else if (targetPlatform == TargetPlatform.iOS || targetPlatform == TargetPlatform.tvOS)
            {
                ...
                {
#if UNITY_2021_2_OR_NEWER
                    var targetArchitecture = (IOSArchitecture) UnityEditor.PlayerSettings.GetArchitecture(NamedBuildTarget.FromBuildTargetGroup(report.summary.platformGroup));
#else
